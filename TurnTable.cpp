...
const unsigned char drum_seq4[] = {5,0,1,0,9,0,5,0, 5,0,1,0,9,4,2,0, 5,0,1,0,9,0,1,4, 1,4,1,0,9,0,2,0,};
...
CTurnTable::CTurnTable() {
    m_discSample = GE::CAudioBuffer::loadWav(QString(":/quitar1.wav"));
...
CTurnTable::~CTurnTable() {
    if (m_audioOut) delete m_audioOut;
...
void CTurnTable::start() {
    if (!m_audioOut) m_audioOut = new GE::AudioOut( this, &m_audioMixer );
...
void CTurnTable::toggleBeat(int index) {
    switch (index) {
    default: m_drumMachine->setSeq(0,0); break;
    case 0:m_drumMachine->setSeq(drum_seq1, 64 );break;
    case 1:m_drumMachine->setSeq(drum_seq2, 16 );break;
    case 2:m_drumMachine->setSeq(drum_seq3, 16 );break;
    case 3:m_drumMachine->setSeq(drum_seq4, 32 );break;
    };
};
